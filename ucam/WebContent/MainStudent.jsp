...
<!-- --------------------------------------------------------------------------------- -->
  ...
  <img src="images\ucamTop.gif" width ="600" height="120" border ="0" onclick="showhide('main');return false;">
   ...
   <nav>
   <ul>
   <li><a href="#"class="button-list" onclick="showhide('one');return false;">수강신청</a></li>
   <li><a href="#">강의</a>
   <ul>
   ...
   <hr></hr>
    ...
    <form name="combo1" action="CourseAdd.jsp">      
    <%
	try
	{
	st = con.createStatement();
	rs = st.executeQuery("select Course_number, Course_name from course");
	}
	catch(SQLException ex)
	{
...
<!-- --------------------------------------------------------------------------------- -->
   ...
   <form name="combo" action="plan.jsp">      
	...
	%>
   <select id="plancombo" name="course_num">
   <%
   while(rs.next())
   {
	   out.println("<option value='"+ rs.getString(1) +"'>" + rs.getString(2) + "</option>");
